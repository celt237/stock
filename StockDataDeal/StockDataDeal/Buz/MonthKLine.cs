...
{
    public class MonthKLine : BuzWeekBase
    {
        public static MonthKLine Instance = new MonthKLine();
        public override string GetTableName(string stockCode)
        {
            return "monthkline_info";
            //return "weekkline_info_" + stockCode.Substring(stockCode.Length - 2);
        }

        

        public IList<Model.WeekStockModel> GetMonthKLineList(string stockcode, IList<Model.StockModel> kLineList, bool powerchange)
        {
            IList<Model.WeekStockModel> oldweekkarr = new List<Model.WeekStockModel>();
            IList<Model.WeekStockModel> weekkarr = GetMonthKLineInfo(stockcode);//从数据抓取获取
            if (!powerchange)
            {
        ...
        private IList<Model.WeekStockModel> GetMonthKLineInfo(string stockcode)
        {
            //获取周期数据
            string url = string.Format("http://hq.stock.sohu.com/cn/{0}/cn_{1}-8.html?uid={2}", stockcode.Substring(stockcode.Length - 3), stockcode, new Random().Next(1000000, 99999999));
            ...
            for (int i = 0; i < collection.Count; i++)
            {
                ...
                if (value.Contains("quote_mk_all") && !value.Contains("init") && !value.Contains("end"))
                {
                    //k线信息
                    //$karr = array_merge($karr, explode(';', KLineHelper::dealKLineStr($arr[1][$i])));
                    value = value.Replace("PEAK_ODIA(", "").Replace("\"", "").Replace("'", "").Replace("[quote_mk_all,", "").Replace("[", "").Replace("],", ";").Replace("]", "").Replace(")", "");
            ...
            {
                Model.WeekStockModel model = ConvertToMonthKLineObject(karr[i]);
                kLineList.Add(model);
            }
            return kLineList;
        }
